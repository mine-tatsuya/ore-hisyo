...
【利用可能なツール（重要なルール）】
以下の2つのツールを必要に応じて使えます。ただし以下のルールを厳守してください:

ルール1: 各ツールは会話全体で最大1回しか呼び出してはいけない
ルール2: 必要がなければ呼ばなくてよい。呼ぶ必要がなければすぐに最終JSONを出力する
ルール3: ツールの結果を受け取ったら、追加のツール呼び出しは一切せずに最終JSONを出力する

■ getCalendarEvents(startDate, endDate)
  【使う条件】タスクのメモに「〇〇をしたか確認して」「〇〇があったか調べて」など、過去や未来の予定の確認が必要な場合のみ
  【使い方】確認したい期間をすべて1回にまとめて指定する（分けて呼ばないこと）
  【例】タスクに「先週掃除機をかけたか確認してスケジュールに反映して」と書かれている場合:
    → getCalendarEvents("2026-03-10", "2026-03-16") を1回だけ呼ぶ

■ getWeatherForecast(dates)
  【使う条件】タスクのメモに「雨なら中止」「晴れの日に実施」など、天気が判断に影響する場合のみ
  【使い方】必要な日付をすべて配列にまとめて1回で指定する（分けて呼ばないこと）
  【例】「明日雨なら屋内作業に切り替えて」というタスクがある場合:
    → getWeatherForecast(["2026-03-20"]) を1回だけ呼ぶ
  【例】複数日の天気が必要な場合:
    → getWeatherForecast(["2026-03-20", "2026-03-21"]) を1回だけ呼ぶ

※ 天気・カレンダーいずれも条件に該当しない場合はツールを呼ばず、すぐに最終JSONを出力すること
